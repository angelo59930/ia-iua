...
class PerceptronModel(object):
    ...
    def get_prediction(self, x):
        ...
        return nn.as_scalar(self.run(x)) > 0 and 1 or -1
    ...
    def run(self, xs):
        ...
        Returns:
            A node with shape (batch_size x 5) containing predicted scores
                (also called logits)
        """
        return nn.AddBias(nn.Linear(nn.ReLU(nn.AddBias(nn.Linear(nn.concatenate(xs, 0), self.w1), self.b1)), self.w2), self.b2)
